...
use crate::audit;
use crate::billing::{ProviderErrorKind, PurgeGate, SubscriptionObservation, SubscriptionProvider};
...
pub(crate) async fn request(
    ...
    if !confirmation {
        return Err(Error::BadRequest(
            "deletion confirmation is required".into(),
        ));
    }
...
pub(crate) async fn advance(
    ...
    match lease.state {
        ...
        DeletionState::Retention => transition_state(pool, lease, DeletionState::Purging).await,
...
async fn advance_billing(
    ...
    let Some(provider) = provider else {
        return fail_attempt(pool, lease, "billing_not_configured").await;
    };
...
async fn handle_provider_error(
    ...
    match provider_retry_decision(kind, lease.attempt_count) {
        RetryDecision::Retry(delay) => schedule_retry(pool, lease, code, delay).await,
        RetryDecision::Failed => {
            fail_attempt(pool, lease, code.as_deref().unwrap_or("provider_error")).await
        }
    }
}
...
async fn purge(pool: &PgPool, lease: &DeletionLease) -> Result<Option<DeletionView>> {
    ...
    let row: Option<PurgeRow> = sqlx::query_as(
        "SELECT d.org_id, d.state, d.subscription_id, d.last_billing_state, \
                d.billing_checked_at IS NOT NULL, d.purge_after <= now(), \
...
mod tests {
    ...
    async fn owner_requests_are_idempotent_and_cancellable() {
        ...
        let first = request(&pool, &org_id, &owner_id, true)
            .await
            .expect("request deletion");
        let repeated = request(&pool, &org_id, &owner_id, true)
            .await
    ...
    async fn worker_reconciles_free_deletion_and_purges_tombstone() {
        ...
        let (org_id, owner_id) = seed_owner(&pool).await;
        let requested = request(&pool, &org_id, &owner_id, true)
            .await
            .expect("request deletion");
